...
- ✅ 版本/清单（software_manifest）：已补齐 `ensure_software_manifest` 工具（GET 最新清单 → 若 snapshot 版本一致则跳过；否则 POST 创建），并以 `POST /api/v1/software-manifests` 落库。
- ✅ 快照前置：已补齐 `ensure_software_manifest_from_snapshot` 工具（先上传 `software_manifest.json` 生成 `manifest_file_version_id`，再创建/更新 `software_manifest` 记录）。
